...
loggerParser.getLoggerNote = function(loggerLine, options){
    const result = {};
    try {
        const contentArr = loggerParser.constant.LOGGER_REG.exec(loggerLine);
...
loggerParser.parseLogger = function(loggerLine, options){
    ...
    const isIgnore = !(options.preserve == null ? isDevelopment : options.preserve);
    if (isIgnore) {
        if (loggerType.input == null) {
            return result.input;
        }
